...
func NewPaymentHandler(app *App) *PaymentHandler {
...
func (h *PaymentHandler) PaymentCallback(c *gin.Context) {
	...
	if transaction.Type == "purchase" && transaction.RelatedType == "plan" {
		planHandler := NewPlanHandler(h.app)
		if err := planHandler.ActivateSubscription(req.OrderID, transaction.UserID, transaction.RelatedID); err != nil {
			logger.Error("激活订阅失败", zap.Error(err))
		}
	}
