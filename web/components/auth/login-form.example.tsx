...
import { authService, useApiMutation } from '@/lib/api';
...
export function LoginFormExample() {
  ...
  // Use API mutation hook for login
  const { mutate: login, isLoading, error } = useApiMutation(
    (data: LoginFormValues) => authService.login({
  ...
  // Use API mutation hook for captcha
  const { mutate: getCaptcha } = useApiMutation(
  ...
  const loadCaptcha = async () => {
    try {
      ...
      if (response.data) {
        ...
        setValue('captchaId', response.data.captcha_id);
      }
    } catch (error) {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    try {
      ...
      // Error is already handled by the useApiMutation hook
